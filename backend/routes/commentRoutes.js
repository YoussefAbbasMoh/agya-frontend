...
router.get("/article/:id", async (req, res) => {
  try {
    // const commentsReply = await Comment.find({}).populate('replies');
    const comments = await Comment.find({ articleId: req.params.id }).populate("userId replies");
...
router.get("/post/:id", async (req, res) => {
  try {
    // const commentsReply = await Comment.find({}).populate('replies');
    const comments = await Comment.find({ postId: req.params.id }).populate("userId replies");
   // console.log(comments)
    res.status(200).json(comments);
  } catch (err) {
    res.status(500).json({ error: err.message });
