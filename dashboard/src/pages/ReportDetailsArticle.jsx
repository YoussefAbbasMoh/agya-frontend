...
export default function ReportDetailsArticle() {
  ...
  if (articleData === undefined) {
    ...
    if (articleData.message === "Article not found") {
      return <div className=" flex justify-center items-center translate-y-60">this data has been deleted</div>
    }
  }
    ...
    if (finalData) {
      ...
      const finalData = await response.json();
        if(finalData) {
          ...
          const response = await fetch(
            `https://agyademo.uber.space/api/articles/${articleData._id}`,
            {
              method: "DELETE",
            }
          );
