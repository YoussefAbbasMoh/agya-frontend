        ...
        <div className=" my-4">
          <TableContainer component={Paper}>
            <Table sx={{ minWidth: 650 }} aria-label="simple table">
              <TableBody>
                {reportData === undefined ? <div className=" block translate-x-[450px] my-16 h-52 w-52 "><PropagateLoader color="#005F6A" size={20} /></div>  :
                 reportData.length === 0 ? (
                  ...
                  reportData.map((report) => {
                    return (
                      <TableRow
                        className={` cursor-pointer ${ report?.isRead === true ?  " pointer-events-none cursor-not-allowed": ""} `}
                        key={report._id}
                        onClick={() =>
                          navigate(
                            `${
                              report.articleId !== undefined
                                ? `/dashboard/report/article/${report.articleId}/${report.content}?id=${report._id}`
                                : report.postId !== undefined
                                ? `/dashboard/report/post/${report.postId}/${report.content}?id=${report._id}`
                                : report.commentId !== undefined
                                ? `/dashboard/report/comment/${report.commentId}/${report.content}?id=${report._id}`
                                : null
                            }`
                          )
                        }
                      ...
                      >
                        <TableCell align="center">
                          <span>
                            ...
                            <img
                              src={
                                report.userImage === "/uploads/default.png" ? "User-60.png": report.userImage 
                ...
                {/* <TableRow
                  ...
                  onClick={()=>navigate('/dashboard/report/comment')}
