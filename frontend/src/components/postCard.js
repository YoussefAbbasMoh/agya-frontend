...
const PostCard = ({ onClick, item }) => {
  ...
  const handleLike = async () => {
    if (!isAuthUser) {
      ...
      toastBC.current.show({
        severity: "error",
        summary: "Please log in to like the post.",
        sticky: true,
      });
      return;
    }
  ...
  const handleDislike = async () => {
    if (!isAuthUser) {
      ...
      toastBC.current.show({
        severity: "error",
        summary: "Please log in to dislike a post.",
        sticky: true,
      });
      return;
    }
  ...
  return (
    <div className="max-w-[400px] md:max-w-xl  w-screen rounded-3xl overflow-hidden shadow-md bg-SoftMain border border-main/50">
      ...
      >
        <div className="flex items-center flex-1">
          <div className="h-12 w-12 mr-3 rounded-full overflow-hidden">
            <img
              src={item.userId?.image === "/uploads/default.png" ? "/User-60.png": item.userId?.image}
              alt="User avatar"
